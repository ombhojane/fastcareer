...
function updateSectionTabs() {
  ...
  const tabIds = ['aboutYouTab', 'coreValuesTab', 'strengthTab', "skillsAssessmentTab", "dreamjobTab", "preferencesTab", "prioritiesTab"]; // Add more as needed
  tabIds.forEach((id, index) => {
    if (index < currentSectionIndex) {
      document.getElementById(id).className = "bg-green-400 text-white text-center py-2 px-4 rounded";
    } else if (index === currentSectionIndex) {
      document.getElementById(id).className = "bg-yellow-400 text-white text-center py-2 px-4 rounded";
    } else {
...
document.getElementById('processBtn').addEventListener('click', displayProcessedResponses);
